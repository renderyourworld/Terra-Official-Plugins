"""
Installer for lmstudio on linux systems.
"""
...
class LMStudioInstaller(Plugin):
    """
    lmstudio installer plugin.
    """

    _version_ = "1.0.0"
    _alias_ = "LM-Studio Installer"
    ...
    tags = ["lmstudio", "editor", "media", "editorial", "kde"]
    fields = [
    ...
    def install(self, *args, **kwargs) -> None:
        ...
        if (
            run(
                f"bash {scripts_directory}/lmstudio-installer.sh {self.download_url} {self.destination}",
                shell=True,
                check=False,
            ).returncode
            != 0
        ):
            raise RuntimeError("Failed to install lmstudio")
